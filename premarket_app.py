# =========================================================
# F&O Universe Premarket (NSE Pre-Open) Tracker — for Google Colab
# =========================================================
...
#
# CHANGE FROM ORIGINAL: instead of tracking only the Nifty50
# index constituents, this pulls NSE's "FO" (Securities in F&O) pre-open bucket
# and then filters it down to exactly the symbols in
# SECURITIES_FO below, so you only see your own watch universe
# (NSE's FNO bucket can include symbols you may not want).
# =========================================================
...
# Cell 1b: Your F&O security universe
SECURITIES_FO = [
...
#   "ALL", "SME", "OTHERS"
# NOTE: the F&O bucket's key is "FO", not "FNO" — "FNO" returns no rows.
...
def print_watchlist(df, top_n=15):
...
# Cell 5c: Filter the fetched universe down to exactly SECURITIES_FO,
# and report any symbols from your list that NSE didn't return data for
# (e.g. newly listed/delisted, or a typo in the list).
def filter_to_fo_universe(df, universe=SECURITIES_FO):
    ...
    if missing:
        print(f"⚠️  {len(missing)} symbol(s) in SECURITIES_FO had no premarket data returned "
...
# Cell 6: Run it
session = get_nse_session()
raw = fetch_preopen_data(session, key="FO")
df = parse_preopen(raw)
df = filter_to_fo_universe(df)
df = df.dropna(subset=["PctChange"]).sort_values("PctChange", ascending=False)

if df.empty:
    raise SystemExit(
        "No usable pre-open data was returned for SECURITIES_FO. "
...
watchlist_df = print_watchlist(df, top_n=15)

TOP_N = 30  # show up to 30 gainers and up to 30 losers
...
print(f"\nUp Stocks (premarket) — showing {len(gainers)} of {stats['up']}:")
print(gainers[["Symbol", "PrevClose", "IEP", "PctChange"]].to_string(index=False))

print(f"\nDown Stocks (premarket) — showing {len(losers)} of {stats['down']}:")
...
def track_live(key="FO", interval_sec=30, duration_min=10):
...
# history_df = track_live(key="FO", interval_sec=30, duration_min=10)
# history_df
...
def plot_premarket(df):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(16, max(12, len(plot_df) * 0.25)),
    ...
    ax1.set_title("F&O Universe Premarket Movement — All Stocks", fontsize=13, fontweight="bold")
